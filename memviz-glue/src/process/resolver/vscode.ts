...
import { ThreadId, StackTrace } from "../memory";
import { ProcessResolver } from "./resolver";
import { ExtensionToMemvizResponse, MemvizToExtensionMsg, RequestId } from "../../messages";
...
export class VsCodeResolver implements ProcessResolver {
    ...
    handleMessage(message: ExtensionToMemvizResponse) {
        ...
        if (resolveFn === undefined) {
            console.warn(`Received response for an unknown or already handled request ${message.requestId}`);
            return;
        }
    ...
    private sendMessage(msg: MemvizToExtensionMsg) {
        console.log(`Sending request to extension (kind=${msg.kind}, id=${msg.requestId})`);
